...
export const load = (async ({locals:{client}, params}) => {
    ...
    const materias = materiasData.filter(materia => materia.id_carrera == parseInt(params.carrera) && materia.semestre == parseInt(params.semestre))
    
    return {materias, carrera: params.carrera, carreras: data.carreras, docentes, semestre: getSemester(params.semestre)};
