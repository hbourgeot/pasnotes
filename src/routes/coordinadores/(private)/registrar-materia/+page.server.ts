...
export const load: PageServerLoad = async ({ locals: { client } }) => {
  ...
  const materias: string[] = dataMat.materias.map((materia: Materia) => (materia.id));
  const materiasAutocomplete: AutocompleteOption[] = dataMat.materias.map((materia: Materia) => ({ label: `${materia.id} - ${materia.nombre}`, value: materia.id, keywords: `${materia.nombre}, ${materia.id}, ${materia.semestre}` }));
