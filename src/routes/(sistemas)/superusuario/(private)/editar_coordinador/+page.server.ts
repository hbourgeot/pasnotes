...
export const load: PageServerLoad = async ({locals:{client,superUsuario}}) => {
  ...
  systemLogger.info(`${superUsuario.nombre} ha entrado a ver los coordinadores registrados y puede que registre uno`)
...
export const actions: Actions = {
  default: async ({ locals: { client, superUsuario }, request }) => {
    ...
    const { ok, status, data } = await client.PUT("/api/coordinacion/update/"+coordinacion.cedula, payload);
    if (!ok) {
