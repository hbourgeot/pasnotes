...
export const load = (async ({ locals: { client } }) => {
  ...
  const materias = {
    ...
    tecnologiaAlimentos: materiasData.filter(
      (materia) => materia.id_carrera == 2
    ),
    comunicacionElectronica: materiasData.filter(
      (materia) => materia.id_carrera == 3
    ),
    disenoGrafico: materiasData.filter((materia) => materia.id_carrera == 4),
    contabilidadCostos: materiasData.filter(
      (materia) => materia.id_carrera == 5
    ),
    administracionBancariaFinanciera: materiasData.filter(
      (materia) => materia.id_carrera == 6
    ),
    administracionEmpresarial: materiasData.filter(
      (materia) => materia.id_carrera == 7
    ),
  };
