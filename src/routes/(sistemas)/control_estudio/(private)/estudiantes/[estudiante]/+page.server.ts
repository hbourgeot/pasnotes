...
export const load: PageServerLoad = async ({ locals: { client, controlEstudio }, params }) => {
  const { ok, data } = await client.GET(`/api/students/${params.estudiante}`);
  if (!ok) return { estudiante: null, notas: [] };

  systemLogger.info(`${controlEstudio.nombre} ha entrado a ver las notas del estudiante con cédula ${params.estudiante}`)
